...
ROOT = Path('/home/meanaverage/sayhi/state/project-intent/evaluations/20260908-luna-release-03-v2')
...
CODEX = Path('/home/meanaverage/sayhi/state/project-intent/evaluations/20260908-luna-release-03/tools/common/codex-bin')
...
ORDER = ('A2-control', 'A2-aware', 'B2-aware', 'B2-control')
LIMITS = {'initial': 1800, 'revision': 1800, 'integration': 1200, 'correction': 900, 'review': 900}
PREFLIGHT_ARMS = ('preflight-v2-control', 'preflight-v2-aware')
REHEARSAL_ARMS = ('rehearsal-control', 'rehearsal-aware')
BASELINE_ARM = 'baseline-calibrated-control'
FILES = ('luna_release_study_v2.py', 'isolation_v2.py', 'LUNA_RELEASE_CONTRACT.md',
         'luna_release_fixture.mjs', 'luna_release_acceptance.mjs', 'luna_release_browser.py', 'luna_release_browser_v2.py',
...
def inputs():
    ...
    names += ['experiments/luna_release_evaluation_v2.py', 'experiments/luna_release_sequence_v2.py', 'experiments/review_bundle.py',
              'docs/CLI_AB_LUNA_RELEASE_03.md', 'docs/CLI_AB_LUNA_RELEASE_03_V2.md']
...
def freeze():
    ...
    prior = ROOT.with_name('20260908-luna-release-03')
    prior_freeze = read_json(prior/'freeze.json')
    if any(sha(SOURCE/name) != expected for name, expected in prior_freeze['inputs'].items()):
        raise ValueError('Original invalidated attempt source was altered')
    repair = read_json(prior/'diagnostics/navigation-v2/execution/result.json')
    if not repair['passed']:
        raise ValueError('Repaired browser calibration did not pass')
    for arm in REHEARSAL_ARMS:
        if not read_json(prior/'arms'/arm/'evaluation/review.json')['valid_review']:
    ...
    write_json(ROOT/'freeze.json',{'at':now(),'inputs':inputs(),'order':ORDER,'limits':LIMITS,
        ...
        'prior_preparation': {'root':str(prior),'freeze_sha256':sha(prior/'freeze.json'),
            'rehearsal_reviews':{arm:sha(prior/'arms'/arm/'evaluation/review.json') for arm in REHEARSAL_ARMS},
            'repair_result_sha256':sha(prior/'diagnostics/navigation-v2/execution/result.json')},
...
def require_frozen(arm):
    ...
    if sha(ROOT/'tools/common/browser/headless_shell.native') != frozen['toolchain']['browser_native_sha256']:
        raise ValueError('Frozen native browser drift')
...
def setup_tools():
    ...
    # Equal CPU-rendering launch wrapper; native binary and wrapper are both frozen.
    binary = common/'browser/headless_shell'
    binary.rename(common/'browser/headless_shell.native')
    write(binary, '#!/bin/sh\nexec "$(dirname "$0")/headless_shell.native" --disable-gpu --disable-software-rasterizer "$@"\n')
    binary.chmod(0o755)
    ...
    write_json(ROOT / 'toolchain.json', {'at': now(), 'model': MODEL, 'effort': 'medium',
        ...
        'browser_native_sha256':sha(common/'browser/headless_shell.native'),
...
Browser check: TRIAL_CHECKOUT=/path/to/status BROWSER_EXECUTABLE={ROOT/'tools/common/browser/headless_shell'} {ROOT/'tools/common/py/bin/python'} {directory/'acceptance/luna_release_browser_v2.py'}
