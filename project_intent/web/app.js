...
function render(result) {
 ...
 const metricDefinitions=[[active.length,'Open workstreams','Workstreams whose durable lifecycle is active, blocked or ready in the selected scope. This does not mean a worker is currently active.','Counts lifecycle state: active + blocked + ready.','intent'],[result.sessions.filter(p=>p.status==='active').length,'Fresh leases','Worker registrations reported active in this observation. A lease is presence metadata, not proof of inference, progress or exclusive ownership.','Counts returned sessions whose observation status is active.','worker'],[result.attention.length,'Attention items','Explainable conditions surfaced by the scoped projection, such as unavailable providers or missing observations. They are not unread messages, severity scores or confirmed failures.','Counts the response’s scoped attention conditions.','handoff'],[result.architecture.filter(a=>a.kind==='invariant'&&a.state==='accepted').length,'Accepted invariants','Architecture records declared as accepted invariants in the current scoped projection, including applicable records imported from another scope. Acceptance is a durable declaration, not verified implementation conformance.','Counts architecture records where kind is invariant and state is accepted.','architecture']];
 for(const [index,[n,label,meaning,counting,name]] of metricDefinitions.entries()){const tile=el('div',undefined,'metric metric-explainer'),trigger=el('button',undefined,'metric-trigger'),panelId='metric-explanation-'+index;trigger.type='button';trigger.setAttribute('aria-expanded','false');trigger.setAttribute('aria-describedby',panelId);trigger.setAttribute('aria-controls',panelId);trigger.setAttribute('aria-label',`${n} ${label}. Show definition.`);trigger.append(icon(name),el('strong',String(n)),el('span',label),el('i','?','metric-help'));const explanation=el('div',undefined,'metric-explanation');explanation.id=panelId;explanation.setAttribute('role','tooltip');explanation.append(el('span','DEFINITION','metric-definition-label'),el('h3',label),el('p',meaning),el('small',counting));const close=()=>{tile.classList.remove('is-open','has-focus');tile.classList.add('is-dismissed');trigger.setAttribute('aria-expanded','false');};trigger.onfocus=()=>{if(!tile.classList.contains('is-dismissed')){tile.classList.add('has-focus');trigger.setAttribute('aria-expanded','true');}};trigger.onblur=()=>{tile.classList.remove('has-focus','is-dismissed');if(!tile.classList.contains('is-open'))trigger.setAttribute('aria-expanded','false');};tile.onmouseleave=()=>tile.classList.remove('is-dismissed');trigger.onclick=()=>{const opening=!tile.classList.contains('is-open');for(const metric of m.querySelectorAll('.metric-explainer')){metric.classList.remove('is-open','has-focus');metric.querySelector('.metric-trigger')?.setAttribute('aria-expanded','false');}tile.classList.toggle('is-dismissed',!opening);tile.classList.toggle('is-open',opening);trigger.setAttribute('aria-expanded',String(opening));};trigger.onkeydown=event=>{if(event.key==='Escape'){close();event.stopPropagation();}};tile.append(trigger,explanation);m.append(tile);}
 ...
 const arch=clear('architecture');for(const a of result.architecture){const row=el('article',undefined,'architecture-row');const identity=el('div',undefined,'architecture-identity'),declaration=el('div',undefined,'architecture-declaration'),reach=el('div',undefined,'architecture-reach');identity.append(el('span',a.kind.toUpperCase()+' · '+a.state+' · r'+a.revision,'identifier'),el('h3',a.key.split(':').slice(1).join(':')));declaration.append(el('p',a.statement));reach.append(el('strong',String(a.workstreams.length)),el('span','applicable workstreams'),scopeName('span',a.scope));row.append(identity,declaration,reach);arch.append(row);}if(!result.architecture.length)empty(arch,'No architectural records enrolled for this scope.');
 ...
 const environments=clear('environments');for(const w of result.workstreams.filter(w=>w.environment.requirement)){const r=w.environment.requirement,row=el('article',undefined,'environment-row'),identity=el('div'),requirement=el('div'),limitations=el('div');identity.append(qualifiedName('div',w.key));requirement.append(el('strong',typeof r==='string'?r:(r.profile||r.requirement||'Declared environment requirement')),el('small','Availability and occupancy unknown'));if(Array.isArray(r.known_limitations)&&r.known_limitations.length){const items=el('ul');for(const item of r.known_limitations)items.append(el('li',String(item)));limitations.append(items);}else limitations.append(el('p','No known limitations recorded.','quiet'));const button=el('button','Inspect');button.onclick=()=>showDetail(w.key);row.append(identity,requirement,limitations,button);environments.append(row);}if(!environments.children.length)empty(environments,'No environment requirements enrolled.');
 const initiatives=clear('initiatives');for(const i of result.initiatives){const item=el('article',undefined,'initiative-item');const top=el('div',undefined,'initiative-top');top.append(scopeName('span',i.scope),el('span',i.status,'identifier'));item.append(top,el('h3',i.title));const members=el('ul');for(const key of i.workstreams)members.append(qualifiedName('li',key));item.append(members);initiatives.append(item);}if(!result.initiatives.length)empty(initiatives,'No visible initiatives supplied by the provider.');
 const handoffs=clear('handoffs');for(const w of result.recent_handoffs){const item=el('article',undefined,'handoff-item');item.append(el('time',new Date(w.handoff_at).toLocaleString(),'identifier'),scopeName('span',w.scope_id),workstreamName('h3',w),el('p',short(w.handoff_summary,260)));const b=el('button','Open handoff');b.onclick=()=>showDetail(w.key);item.append(b);handoffs.append(item);}if(!result.recent_handoffs.length)empty(handoffs,'No handoffs recorded.');
 const sources=clear('sources');for(const s of scopes){const item=el('article',undefined,'source-item'),top=el('div',undefined,'source-item-top');top.append(scopeName('span',s.id),el('span',s.provider_status,'source-state'));item.append(top,el('h3',s.label),el('p',s.source?'Snapshot '+s.source.captured_at+' · '+s.age_seconds+'s old':'No cached durable state'),el('dl',undefined,'source-facts'));const facts=item.querySelector('.source-facts');facts.append(el('dt','Execution'),el('dd',s.execution.status),el('dt','Meaning'),el('dd',s.execution.meaning));sources.append(item);}
}
