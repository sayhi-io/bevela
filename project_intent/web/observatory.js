...
function selectView(focus=false){const key=location.hash.slice(1)||'home';const selected=views[key]?key:'home';document.documentElement.dataset.currentView=selected;document.querySelector('main').dataset.currentView=selected;for(const section of document.querySelectorAll('[data-view]'))section.hidden=!section.dataset.view.split(' ').includes(selected);for(const a of $('view-nav').children){if(a.hash==='#'+selected)a.setAttribute('aria-current','page');else a.removeAttribute('aria-current');}for(const [i,card] of [...$('rested').children].entries())card.hidden=selected==='home'&&i>=6;for(const [i,card] of [...$('convergence').children].entries())card.hidden=selected==='home'&&i>=3;$('view-title').textContent=views[selected][0];$('view-title').classList.toggle('home-navigation-target',selected==='home');$('view-description').textContent=views[selected][2];$('view-description').hidden=selected==='home';if(focus)$('view-title').focus();}
