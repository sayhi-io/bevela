...
function renderCalendarEvidence(){
 ...
 for(const event of filtered){const row=el('tr');row.dataset.eventType=event.type;row.dataset.at=String(event.at);const type=el('td'),evidence=el('td'),recorded=el('td'),actions=el('td');type.dataset.label='Type';evidence.dataset.label='Evidence';recorded.dataset.label='Recorded';actions.dataset.label='Actions';type.append(el('span',event.label,'board-state '+event.type));if(event.work)evidence.append(qualifiedName('div',event.work.key));else evidence.append(el('div',`${event.scope} · ${event.commit.repository}`,'identifier'));evidence.append(el('p',event.summary));if(event.type==='commit')evidence.append(el('small',`${event.commit.oid.slice(0,12)} · ${event.commit.publication}`,'identifier'));recorded.append(el('time',displayTime(event.at),'identifier'));if(event.type==='pull-request'){const link=pullRequestChip(event.ref);if(link)actions.append(link);}if(event.work)actions.append(workButton(event.work,'Open Workstream'));if(!actions.children.length)actions.append(el('span','Local record','quiet'));row.append(type,evidence,recorded,actions);body.append(row);}
 if(!filtered.length){const row=el('tr',undefined,'calendar-ledger-empty'),cell=el('td',selectedCalendarEvents.length?'No evidence matches these filters.':'Choose a marked day to review recorded work.');cell.colSpan=4;row.append(cell);body.append(row);}
}
function renderCalendar(result,focus=false){
 ...
 const selected=facts.filter(event=>event.date===calendarSelected),selectedDate=new Date(calendarSelected+'T12:00:00');$('calendar-day-title').textContent=Number.isFinite(selectedDate.getTime())?selectedDate.toLocaleDateString(undefined,{dateStyle:'full'}):'No day selected';$('calendar-day-summary').textContent=selected.length?`${selected.length} dated record${selected.length===1?'':'s'} in the available scoped response.`:'No dated evidence in the available scoped response. This is not evidence of inactivity.';selectedCalendarEvents=selected;renderCalendarEvidence();
...
for(const id of ['calendar-event-search','calendar-event-type','calendar-event-sort'])$(id).addEventListener(id==='calendar-event-search'?'input':'change',renderCalendarEvidence);
let detailKey=null;
