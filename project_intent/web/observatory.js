...
function renderCalendar(result,focus=false){
 ...
 const gitCoverage=result.local_git?.status||'not authorized/configured';$('calendar-coverage').textContent=`Available evidence: ${handoffs} provider handoffs · ${reports} worker reports · ${releases} recent release observations · ${prs} pull request observations · ${commits} local Git commits. Local Git coverage: ${gitCoverage}. Empty days mean no dated evidence in this scoped response; they are not evidence that no work occurred.`;
 const grid=clear('calendar-grid');for(const day of view.days){const button=el('button',undefined,'calendar-day');button.type='button';button.dataset.date=day.key;button.setAttribute('aria-pressed',String(day.key===calendarSelected));button.classList.toggle('outside-month',!day.inMonth);button.classList.toggle('today',day.key===today);button.append(el('span',String(day.day),'calendar-day-number'));const counts={};if(day.events.length){const types=el('span',undefined,'calendar-event-marks');for(const type of ['handoff','report','release','pull-request','commit']){const count=day.events.filter(event=>event.type===type).length;counts[type]=count;if(count){const mark=el('i',String(count),'event-mark '+type);mark.title=ActivityViewsFacts.eventLabels[type];types.append(mark);}}button.append(types);}const evidence=day.events.length?` · ${counts.handoff||0} handoffs, ${counts.report||0} worker reports, ${counts.release||0} recent releases, ${counts['pull-request']||0} pull request observations, ${counts.commit||0} Git commits`:' · no dated evidence';button.setAttribute('aria-label',day.date.toLocaleDateString(undefined,{dateStyle:'full'})+evidence);button.onclick=()=>{if(!day.inMonth)calendarMonth=new Date(day.date.getFullYear(),day.date.getMonth(),1,12);selectCalendarDay(day.key,true);};grid.append(button);}
